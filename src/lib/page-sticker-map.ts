...
export function findSectionsForPage(pagePath: string, albumYear: number): Section[] {
  ...
  if (pageTeam.includes("result")) {
    return stickerAlbum.sections.filter(
      (s) => normalize(s.name).includes("result") || normalize(s.name).includes("statistic")
    );
  }
  if (pageTeam.includes("stadium")) {
    return stickerAlbum.sections.filter(
      (s) => normalize(s.name).includes("stadium") || normalize(s.name).includes("venue")
    );
  }
  ...
  for (const section of stickerAlbum.sections) {
    ...
    for (const [, aliases] of Object.entries(NAME_ALIASES)) {
      ...
      const sectionMatch = aliases.some((alias) => sectionNorm.includes(alias));
...
export function getStickersForPage(pagePath: string, albumYear: number): Sticker[] {
...
export function getSectionNameForPage(pagePath: string, albumYear: number): string | null {
  const sections = findSectionsForPage(pagePath, albumYear);
