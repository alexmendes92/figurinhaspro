...
export default function PrecosPage() {
  ...
  const [sellerPlan, setSellerPlan] = useState<string>("FREE");
  ...
  useEffect(() => {
    Promise.all([
      ...
      fetch("/api/seller").then((r) => r.json()),
    ]).then(([priceData, sellerData]) => {
      setRules(priceData);
      setSellerPlan(sellerData.plan || "FREE");
      setLoading(false);
    });
  }, []);
        ...
        <>
          ...
          <div className="mt-8">
            ...
            {!canUseAlbumRules ? (
              ...
              /* Lista de regras por álbum */
              <div className="space-y-3">
                {Array.from(albumRulesBySlug.entries()).map(([slug, slugRules]) => {
                  const albumInfo = albums.find((a) => a.slug === slug);
                  return (
