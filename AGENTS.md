...
- **Plan guards**: `src/lib/plan-limits.ts` exporta `checkStickerLimit()`, `checkOrderLimit()`, `checkAlbumLimit()`, `hasFeature()`. **Ativos** — consultam DB e retornam `{ allowed, current, max }`. Callers reais: `api/orders`, `api/bot/quote`, `api/inventory`, `api/prices`. Limites em `PLAN_LIMITS` (FREE=100 stickers/1 album/10 orders-mes, PRO=1000/13/100, UNLIMITED=∞/13/∞). Cockpit comercial tem tarefas antigas "Restaurar checkXXX" em `/painel/comercial/iniciativas` — ja foram restaurados, marcar DONE quando user confirmar.
