...
export default function PedidosClient({ shopUrl }: PedidosClientProps) {
  ...
  return (
    <div className="p-6 lg:p-8 max-w-5xl slide-up">
      ...
      {loading ? (
        ...
        <EmptyOrdersKit shopUrl={shopUrl} />
      ) : filtered.length === 0 ? (
        <div className="p-16 rounded-2xl border border-[var(--border)] bg-[var(--card)] text-center">
          ...
          <p className="text-sm text-[var(--muted)]">Nenhum pedido com esse filtro</p>
          <p className="text-xs text-zinc-600 mt-1">Tenta outro status ou limpa a busca</p>
        </div>
      ) : (
        <div className="space-y-3">
          {filtered.map((order) => {
