...
function getPrice(
  ...
  // Preço customizado > regra de preço > padrão
...
export default function StoreAlbumView({
  ...
  // Filtra só figurinhas em estoque
  const availableStickers = section.stickers.filter(
    ...
    <div className="min-h-screen bg-zinc-950 flex flex-col">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-zinc-500 font-[family-name:var(--font-geist-mono)]">
                {sellerName} · {Object.keys(stockMap).length} disponíveis
              </p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 lg:p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-xs text-zinc-500">
              {availableStickers.length} disponíveis
            </span>
          </div>

          {availableStickers.length === 0 ? (
            <p className="text-zinc-500 text-sm">Nenhuma figurinha desta seção em estoque.</p>
          ) : (
              ...
              {availableStickers.map((sticker) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="px-1.5 py-1 bg-zinc-900/90 flex items-center justify-between">
            ...
            <div className="flex-1 overflow-y-auto">
              {cart.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full text-center px-8">
                  <p className="text-zinc-500 text-sm">Nenhuma figurinha selecionada</p>
                ...
                <div className="divide-y divide-zinc-800/50">
                  {cart.map((item) => (
                    <div key={item.sticker.code} className="flex items-center gap-3 px-4 py-3">
                      <div className="relative w-9 h-13 rounded overflow-hidden border border-zinc-700 shrink-0">
                        <Image src={item.sticker.image} alt={item.sticker.name} fill className="object-cover" sizes="36px" />
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-medium truncate">{item.sticker.name}</p>
                        <p className="text-[10px] text-zinc-500 font-[family-name:var(--font-geist-mono)]">{item.sticker.code}</p>
                      </div>
                      <span className="text-sm font-[family-name:var(--font-geist-mono)] text-amber-400">
                        R${(item.price * item.quantity).toFixed(2).replace(".", ",")}
                      </span>
                      <button onClick={() => removeFromCart(item.sticker.code)} className="text-zinc-600 hover:text-red-400">
                        <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                          <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
                        </svg>
                      </button>
            ...
            {cart.length > 0 && (
              <div className="border-t border-zinc-800 p-4 space-y-3">
                <div className="flex items-center justify-between">
                  <span className="text-sm text-zinc-400">{cart.length} figurinhas</span>
      ...
      {showCheckout && (
        <div className="fixed inset-0 z-[95] flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
          <div className="bg-zinc-900 border border-zinc-700 rounded-2xl max-w-sm w-full p-6" onClick={(e) => e.stopPropagation()}>
            <h3 className="text-lg font-semibold mb-4">Finalizar Orçamento</h3>
            <form
              onSubmit={(e) => {
                e.preventDefault();
            ...
            >
              <div>
                ...
                <input
                  name="name"
                  required
                  className="w-full px-3 py-2.5 rounded-lg bg-zinc-800 border border-zinc-700 text-white text-sm focus:outline-none focus:border-amber-500/50"
              ...
              <div className="p-3 rounded-lg bg-zinc-800/50 border border-zinc-800">
                <div className="flex justify-between text-sm">
                  <span className="text-zinc-400">{cart.length} figurinhas</span>
                  <span className="font-bold text-amber-400 font-[family-name:var(--font-geist-mono)]">
                    R${cartTotal.toFixed(2).replace(".", ",")}
                  </span>
                </div>
              </div>
              ...
              <div className="flex gap-2">
                ...
                <button
                  type="submit"
                  className="flex-1 py-2.5 rounded-xl bg-green-600 hover:bg-green-500 text-white font-semibold text-sm transition-colors flex items-center justify-center gap-2"
                >
                  {sellerPhone ? (
                    <>
      ...
      {cart.length > 0 && !showCart && !showCheckout && (
        <div className="fixed bottom-0 left-0 right-0 z-40 p-3 bg-zinc-950/90 backdrop-blur-sm border-t border-zinc-800 lg:hidden">
          <button
            onClick={() => setShowCart(true)}
            className="w-full py-3 rounded-xl bg-amber-500 text-black font-semibold text-sm flex items-center justify-center gap-2"
          >
            Ver orçamento ({cart.length}) — R${cartTotal.toFixed(2).replace(".", ",")}
          </button>
        </div>
      )}
    </div>
