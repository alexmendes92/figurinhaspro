...
export default function PainelShell({ seller, children, pendingOrders = 0, isAdmin = false }: { seller: SellerInfo; children: React.ReactNode; pendingOrders?: number; isAdmin?: boolean }) {
  ...
  const navItems = isAdmin
    ? [
        ...nav,
        { href: "/painel/comercial", label: "Comercial", icon: "M3 13.125C3 12.504 3.504 12 4.125 12h2.25c.621 0 1.125.504 1.125 1.125v6.75C7.5 20.496 6.996 21 6.375 21h-2.25A1.125 1.125 0 013 19.875v-6.75zM9.75 8.625c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125v11.25c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 01-1.125-1.125V8.625zM16.5 4.125c0-.621.504-1.125 1.125-1.125h2.25C20.496 3 21 3.504 21 4.125v15.75c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 01-1.125-1.125V4.125z" },
        { href: "/painel/admin/revendedores", label: "Revendedores", icon: "M15 19.128a9.38 9.38 0 002.625.372 9.337 9.337 0 004.121-.952 4.125 4.125 0 00-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 018.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0111.964-3.07M12 6.375a3.375 3.375 0 11-6.75 0 3.375 3.375 0 016.75 0zm8.25 2.25a2.625 2.625 0 11-5.25 0 2.625 2.625 0 015.25 0z" },
      ]
    : nav;
    ...
    <div className="flex min-h-screen bg-[#0b0e14]">
      ...
      >
        ...
        <div className={`h-16 flex items-center border-b border-white/[0.06] ${collapsed ? "px-3 justify-center" : "px-5 gap-3"}`}>
          <Link href="/painel" className="w-9 h-9 rounded-xl bg-gradient-to-br from-amber-400 to-amber-500 flex items-center justify-center shadow-lg shadow-amber-500/20 shrink-0 hover:shadow-amber-500/30 transition-shadow hover:scale-105 active:scale-95">
          ...
          {!collapsed && (
            <div className="min-w-0">
              ...
              <p className={`text-[10px] font-semibold uppercase tracking-wider ${plan.color}`}>{plan.label}</p>
            </div>
          )}
        </div>
          ...
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                ...
                <svg className={`w-[18px] h-[18px] shrink-0 transition-colors ${active ? "text-amber-400" : "text-gray-500 group-hover:text-gray-300"}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
        ...
        <div className={`border-t border-white/[0.06] space-y-1 ${collapsed ? "p-1.5" : "p-3"}`}>
          {!collapsed && (
            <Link href={`/loja/${seller.shopSlug}`} target="_blank" className="flex items-center gap-2.5 px-3 py-2 rounded-xl text-[12px] text-gray-500 hover:text-amber-400 hover:bg-amber-500/5 transition-all group">
              <svg className="w-4 h-4 group-hover:rotate-[-12deg] transition-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
      ...
      {mobileOpen && <div className="fixed inset-0 z-40 bg-black/60 lg:hidden backdrop-blur-sm" onClick={() => setMobileOpen(false)} aria-hidden="true" />}
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="h-14 border-b border-white/[0.06] bg-[#0b0e14]/80 backdrop-blur-xl flex items-center px-4 sm:px-5 gap-3 sticky top-0 z-30">
          <button onClick={() => setMobileOpen(true)} className="lg:hidden w-9 h-9 rounded-lg border border-white/10 flex items-center justify-center text-gray-400 active:bg-white/5 hover:border-white/20 transition-colors" aria-label="Abrir menu">
          ...
          <Link href={`/loja/${seller.shopSlug}`} target="_blank" className="ml-auto hidden sm:flex items-center gap-1.5 text-[11px] text-gray-500 hover:text-amber-400 transition-colors">
